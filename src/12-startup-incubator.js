...
export class Incubator {
  ...
  constructor(name, maxStartups) {
    if (maxStartups <= 0) {
      throw new Error("Invalid Startups");
    }
    this.name = name;
  ...
  async assignMentor(startupName, mentor) {
    return new Promise((resolve, reject) => {
      const exist = this.#startups.some((s) => s.startup.name === startupName);
      if (!exist) {
  ...
  async batchProcess(startups) {
    const promises = startups
      .map((start) => this.admitStartup(start))
