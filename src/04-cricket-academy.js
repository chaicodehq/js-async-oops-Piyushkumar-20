...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    for (const inning of this.innings) {
      totalstrikeRate += inning.strikeRate;
    }
...
export class AllRounder extends Player {
  ...
  getProfile() {
    const totalRuns = this.innings.reduce((sum, inning) => sum + inning.runs, 0);
    const totalWickets = this.spells.reduce(
      (sum, spell) => sum + spell.wickets, 0
    );
    return {
      ...super.getProfile(),
      battingStyle: this.battingStyle,
